...
double DF( const double v_R, const double v_theta, const double v_z, const double z )
{
  ...
  return factor * (DF_v_R * DF_v_theta * DF_v_z * DF_z) / (sigma_R * sigma_theta * sigma_z);
}

// double f_gaussian( const double v_R, const double v_theta, const double v_z )
// {
//   return exp( - 0.5 * (v_R * v_R / (sigma_R * sigma_R) + v_theta * v_theta / (sigma_theta * sigma_theta) + v_z * v_z / (sigma_z * sigma_z)) ); 
// }
...
// 	}
	...
	//for (size_t i = 0; i < (num_rows / num_pts); i ++)
	{
	  ...
	  // const double f = f_gaussian( v_R, v_theta, v_z );
	  ...
	  dLdt_x[R] += f * vy * Lx_exchange * dz * dvx * dvy * dvz;
	  ...
	  //dLdt_z[R] += Lz_exchange / t;
         ...
         dLdt_z_total += f * vy * Lz_exchange * dx * dz * dvx * dvy * dvz; //need to multiply by (integral over dy)=const
       }
      ...
      for (map<double, double>::const_iterator i = dLdt_x.begin(); i != dLdt_x.end(); i ++)
	{
	  ...
	  cout << current_x << " " << current_dLdt_x << " " << current_dLdt_y << " " << current_dLdt_z << endl; //<< current_dLdt_z / (num_pts * num_pts * num_pts * num_pts) << endl;
      ...
      cout << "# " << dLdt_x_total << " " << dLdt_y_total << " " << dLdt_z_total << endl;
