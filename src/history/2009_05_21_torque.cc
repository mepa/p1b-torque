...
const double kappa = 1.8 * Omega0; // <--- VARY
const double nu = 2.0 * kappa; // <--- VARY (disk thickness test)
...
// double f_gaussian( const double v_R, const double v_theta, const double v_z )
// {
// //   const double n0 = 1.0e8;
// //   const double factor = n0 * pow(2.0 * M_PI, - 3.0 / 2.0) / (sigma_R * sigma_theta * sigma_z);

//   return exp( - 0.5 * (v_R * v_R / (sigma_R * sigma_R) + R * v_theta * R * v_theta / (sigma_theta * sigma_theta) + v_z * v_z / (sigma_z * sigma_z)) );  //* factor;
// }
		  ...
		  for (size_t ivz = 0; ivz < num_pts; ivz++)
		    {
		      ...
		      if ((R == 0) && (rank == 0)) cout << "# (R, R_theta, z, v_R, v_R_theta, v_z) = (" << R << ", " << R_theta << ", " << z << ", " << v_R << ", " << v_R_theta << ", " << v_z << ")" << endl;
		      ...
		      if ( (R == R_cl) && (z == 0.0) && (v_R == 0.0) && (v_R_theta == 0.0) && (v_z == 0.0) )
			{
		      ...
		      const double pot_host_i = - G_M_host / r_total_i;
...
// 			  if (t > 1400.0)
// 			    {
// 			      //if (rank == 0) cout << "# (" << ix << " " << iz << " " << ivx << " " << ivy << " " << ivz << ") " << "Integration too long!" << endl;
// 			      break;
// 			    }
...
// 			    }		
		      ...
		      //const double pot_host_f = - ( Omega0_sq * R_cl * R_cl + 0.5 * (kappa * kappa - 3.0 * Omega0_sq) * (r_total - R_cl) * (r_total - R_cl) ) * x / r_total;
		      const double pot_host_f = - G_M_host / r_total_f;
...
// 		      if ( (E_J_final - E_J_initial) > 1.0e-8 ) 
// 			{
// 			  if (rank==0) 
// 			    {
...
// 		      results.E_J = E_J_final;
		      ...
		      const double E_J_crit = 1.0e-5;
		      const double E_J_initial = 0.5 * p_sq_i + pot_host_i - Omega0 * Lz_initial;
		      const double E_J_final = 0.5 * p_sq_f + pot_host_f - Omega0 * Lz_final;
		      if ( (E_J_final - E_J_initial) > E_J_crit )
			{
			  if (rank==0)
			    {
			      ofstream errorFile( "ErrorFile.log" );
			      cerr.rdbuf( errorFile.rdbuf() );
			      cerr << "Delta(Energy) > " << E_J_crit << " for " << ix << " " << iz << " " << ivx << " " << ivy << " " << ivz << ": " << E_J_final - E_J_initial << endl;
			    }
			}
		      results.E_J = E_J_final - E_J_initial;
		      // END TEMPORARY CODE
  ...
  for (int irank = 0; irank < num_procs; irank ++)
    {
      ...
      //cout << "rank " << rank << " doing gather " << endl;
      ...
      //cout << "rank " << rank << " done gather " << ierr_1 << endl;
