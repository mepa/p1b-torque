...
const size_t num_pts = 24; // <-- CHANGE
const size_t num_steps = num_pts - 1;

//------------------------------
// Cluster parameters:

const double M_cl = 1.0e-1; // <-- CHANGE
...
// 	}
	...
	//for (size_t i = 0; i < (num_rows / num_pts); i ++)
	{
	  ...
	  //	  if(R < 0.1) { 
	  //	    cout << i << " " << t << " " << R << " " << z << " " << v_R << " " << v_R_theta << " " << v_z << " " << Lx_exchange << " " << Ly_exchange << " " << Lz_exchange << endl;
	  //	    abort();
	  //	  }
	  ...
	  //cout << dLdt_z[R] / ( G * G * M_cl * M_cl / (sigma_R * sigma_R) ) << " " << dLdt_z_total / ( G * G * M_cl * M_cl / (sigma_R * sigma_R) ) << endl; //DEBUG
	...
	{
	  //cout << i->first << " is the iterator" << endl;
